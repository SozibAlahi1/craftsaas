...
import { ChevronRight, ChevronUp, Clock, Facebook, Mail, MapPin, Phone, Youtube } from 'lucide-react';
...
export function ShutkirFooter() {
    ...
    const footerDescription =
        settings?.footer_description || 'বাংলাদেশের সেরা অর্গানিক শুকটি মাছের অনলাইন বাজার। ভেজালমুক্ত পণ্য সরাসরি আপনার দরজায়।';
    ...
    const renderLink = (link: { label: string; url: string }) => {
        ...
        return (
            <Link href={href} className={cls} style={{ color: P.textMuted }}>
                {link.label}
            </Link>
        );
    };
        ...
        <footer style={{ background: P.bg, borderTop: `1px solid ${P.border}` }} className="font-sans">
            <div className="mx-auto max-w-[1440px] px-4 pt-16 pb-8 sm:px-6 lg:px-8">
                <div className="grid gap-12 lg:grid-cols-[1.2fr_1fr_1fr_1.2fr]">
                    ...
                    <div>
                        <Link href={route('home')} className="mb-5 inline-flex items-center gap-3">
                            {logoUrl ? (
                                <img src={logoUrl} alt={siteName} className="h-12 w-auto object-contain" />
                            ) : (
                                <span className="text-2xl font-black tracking-tight uppercase" style={{ color: P.textMain }}>
                                    {siteName}
                                </span>
                            )}
                        </Link>
            ...
            <button
                ...
                className="fixed right-6 bottom-6 z-50 inline-flex h-11 w-11 items-center justify-center rounded-full shadow-lg transition-transform hover:-translate-y-1"
