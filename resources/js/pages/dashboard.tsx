...
export default function Dashboard({ stats, weeklyData, monthlyData, salesByCategory, recentOrders, topProducts }: Props) {
    ...
    const filteredOrders = orderFilter === 'All' ? recentOrders : recentOrders.filter((o) => o.status === orderFilter.toLowerCase());
                    ...
                    {statCards.map((card) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="absolute -top-5 -right-5 h-20 w-20 rounded-full opacity-60" style={{ background: card.circle }} />
                                <div className="absolute -top-8 -right-2 h-16 w-16 rounded-full opacity-40" style={{ background: card.circle }} />

                                <div className="relative flex flex-col gap-3">
                                    {/* Icon */}
                                    <div className="inline-flex h-9 w-9 items-center justify-center rounded-lg" style={{ background: card.iconBg }}>
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-0.5 text-[10px] text-slate-400">{card.noTaka ? 'total count' : 'total amount'}</p>
                                    </div>
                                </div>
                            </div>
                        );
                                        ...
                                        {salesByCategory.map((cat, i) => (
                                            <div key={i} className="flex items-center justify-between text-xs">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    />
                                                    <span className="max-w-[100px] truncate font-medium text-slate-600">{cat.name}</span>
                                                </div>
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2">
                                                <p className="truncate text-xs font-bold text-slate-800">{order.full_name}</p>
                                                <span
                                                    className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold capitalize ${STATUS_BADGE[order.status] || 'bg-slate-100 text-slate-600'}`}
                                                >
                                                    {order.status}
                                                </span>
                                            </div>
                    ...
                    <div className="overflow-hidden rounded-xl border border-slate-100 bg-white shadow-sm">
                        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                            ...
                            <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-bold text-slate-600">by quantity</span>
                        </div>
                            ...
                            <div className="divide-y divide-slate-50 p-2">
                                {topProducts.map((product, idx) => (
                                    ...
                                    >
                                        {/* Rank */}
                                        <div
                                            className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-black text-white ${idx === 0 ? 'bg-amber-400' : idx === 1 ? 'bg-slate-400' : idx === 2 ? 'bg-orange-400' : 'bg-slate-200 text-slate-600'}`}
                                        >
                                            {idx + 1}
                                        </div>
                                        {/* Image */}
