...
import { Edit2, ExternalLink, Eye, EyeOff, Move, Plus, Trash2 } from 'lucide-react';
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-black tracking-tight text-slate-950 uppercase">Featured Tiles</h1>
                        <p className="text-sm font-medium text-slate-500">Manage the 8 grid tiles on the home page</p>
                    </div>
                    <Link
                        ...
                        <table className="w-full border-collapse text-left">
                            <thead>
                                <tr className="border-b border-slate-100 bg-slate-50/50">
                                    ...
                                    <th className="px-6 py-4 text-xs font-black tracking-widest text-slate-400 uppercase">Title / Link</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {tiles.map((tile) => (
                                    <tr key={tile.id} className="group transition-colors hover:bg-slate-50/30">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="leading-tight font-black text-slate-900">{tile.title || 'Untitled Tile'}</div>
                                            {tile.link && (
                                                <div className="mt-1 flex items-center gap-1 text-[10px] font-bold tracking-widest text-slate-400 uppercase">
                                ...
                                {tiles.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-12 text-center">
                                            <div className="text-sm font-bold tracking-widest text-slate-400 uppercase">No tiles found.</div>
                                            <Link
                                                href={route('admin.featured-tiles.create')}
